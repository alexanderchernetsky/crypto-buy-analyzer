...
const LiquidityPoolsPage: React.FC = () => {
    ...
    const filteredPools = useMemo(() => showClosed ? pools : pools.filter(p => p.status !== 'closed'), [pools, showClosed]);
    const sortedPools = useMemo(() => [...filteredPools].sort((a, b) => new Date(b.earningRows[0].startDate).getTime() - new Date(a.earningRows[0].startDate).getTime()), [filteredPools]);
                        ...
                        <div className="flex flex-wrap gap-2">
                            <button
                                ...
                                className={`cursor-pointer inline-flex items-center gap-1 rounded-md px-3 py-1.5 text-sm font-semibold text-white transition
                                    ...
                                    : 'bg-slate-600 hover:bg-slate-700'} disabled:opacity-50 disabled:cursor-not-allowed`}
                            >
