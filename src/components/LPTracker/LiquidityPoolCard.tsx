		...
		>
			...
			<div className="flex justify-between items-center mb-6">
				...
				{formData.status === 'closed' && (
					<span className="px-3 py-1 bg-slate-700 text-slate-200 text-sm font-semibold rounded-md">Closed</span>
				)}
			</div>
			...
			<div className="space-y-4">
				...
				<div className="flex flex-col gap-2 mb-4">
					...
					<div className="flex items-center gap-4">
						...
						<div className="flex-1 relative h-4 bg-slate-700 rounded-full">
							<div
								...
								style={{
									left: `${Math.max(
										0,
										Math.min(100, ((price - formData.rangeFrom) / (formData.rangeTo - formData.rangeFrom)) * 100),
									)}%`,
								}}
						...
						>
							{price >= formData.rangeFrom && price <= formData.rangeTo ? `Within range` : `Outside range`}
						</span>
					</div>
